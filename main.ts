...
class student {
  ...
  enrollCourses(courses: string) {
    this.courses.push(courses);
  }

  balanceCheck() {
    console.log(
      `This Student's Name is "${this.name}" and her/has Balance is $${this.balance}`
    );
  }
  ...
  payFees(fees: number) {
    this.balance -= fees;
    console.log(
      `The $${fees} Fees has been paid and The Outstanding Balance is $${this.balance}`
    );
  }
...
class studentManager {
  ...
  addStudents(name: string) {
    ...
    console.log(
      `Student : "${name}" added sucessfully, Student ID is : "${student_.id}" `
    );
  }

  enrollStudents(studentId: number, course: string) {
  ...
  payStudentFees(studentId: number, fees: number) {
    let findStudent = this.findStudentMethod(studentId);
    if (findStudent) {
  ...
  findStudentMethod(studentId: number) {
  ...
  while (true) {
    ...
    switch (choises.choise) {
      ...
      case "Enroll Student":
        ...
        Student_Management.enrollStudents(
